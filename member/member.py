...
def api_user_signup():
    try:
        ...
        cursor.execute("SELECT * FROM `taipeitrip_member` WHERE `email`=%s",[usermail])
...
def api_user_signin():
    try:
        ...
        connection_object = connection_pool.get_connection()
        cursor = connection_object.cursor(dictionary=True)
        cursor.execute("SELECT * FROM `taipeitrip_member` WHERE `email`=%s AND `password`=%s;",[usermail,password])
        result=cursor.fetchone()
        print(result)
        if result == None:
            return jsonify({
